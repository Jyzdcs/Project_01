...
randomIndex = randomNumberFunc(9);

function launch(imageOne, imageTwo, imageTree, imageFour, bdnQuestions, questionRandom) {
    let indexOfEl = randomNumberFunc(9) // selected the random object in the array
        
    let arrayUrlRandom = [
        bdnQuestions[indexOfEl].url1,
        bdnQuestions[indexOfEl].url2,
        bdnQuestions[indexOfEl].url3,
        bdnQuestions[indexOfEl].response
    ];

    function randomUrlFunc(array, image) {
        let urlRandom = array[randomNumberFunc(array.length)]; // stocker un el au hasard parmis arrayUrlRandom
    ...
    questionRandom.textContent = bdnQuestions[indexOfEl].question; // print the question
...
let test = bdnQuestions.filter((obj) => obj != responseValue)
console.log(test)
